...
class Trie:
    ...
    def print_levels(self):
        """
        prints the trie nodes in in level order
        """
        queue = [self.root]
        while queue:
            current = queue.pop(0)
            print 'node={}'.format(current.__dict__)
            if current.children:
...
def matrix_search_trie(matrix, words_dict):
    ...
    print 'Trie Matrix Search ----'
    words_trie = Trie()
    for word in words_dict.keys():
        words_trie.insert(word.lower())
    words_trie.print_levels()    
...
def main():
    ...
    keys = ['the', 'these', 'their', 'thaw', 'ana', 'b']
    for k in keys:
        print '{} --> {}'.format(k, t.search(k))
