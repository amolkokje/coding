...
class Solution(object):
    def numOfBurgers(self, tomatoSlices, cheeseSlices):
        ...
        def _recurse(total_jumbo, total_small, tomato, cheese):
            #print 't={}, c={}'.format(tomato, cheese)

            if tomato==0 and cheese==0:
                ele = [total_jumbo, total_small]
                if ele not in possibilities:
                    possibilities.append(ele)

            if tomato<2 and cheese<1:
                return

            if tomato>=2 and cheese>=1:
                _recurse(total_jumbo, total_small+1, tomato-2, cheese-1)

            if tomato>=4 and cheese>=1:
                _recurse(total_jumbo+1, total_small, tomato-4, cheese-1)


        _recurse(0, 0, tomatoSlices, cheeseSlices)
